"""Local PDF signature inspection for Customs Declaration originals.

Detects embedded SubFilter variants (including MoldSign) and checks CMS
integrity over /ByteRange with OpenSSL ``cms -verify -noverify``.
...
def _openssl_cms_verify(signed: bytes, cms_der: bytes) -> tuple[bool, str]:
	try:
		...
		raise SadImportError("OpenSSL is required to verify PDF signatures") from exc
	except subprocess.TimeoutExpired as exc:
		from erpnext_moldova_customs.utils.sad_pdf import SadImportError

		raise SadImportError("PDF signature verification timed out") from exc
